...
read(std::string_view s) noexcept requires std::same_as<T, u32> or std::same_as<T, u64> {
  if (s.empty()) return {};
...
[[gnu::always_inline, gnu::flatten]] inline T
read(std::string_view s) noexcept requires std::same_as<T, i32> or std::same_as<T, i64>{
  if (s.empty()) return {};
...
read(std::string_view view, Ts &...vals) noexcept {
  ...
  iterate<FormatStr>([&]<sz CurrIndex, char CurrChar> [[gnu::flatten, gnu::always_inline]] () {
    if constexpr (is_placeholder(CurrChar)) {
      ...
      sz const len = view.find_first_of(FormatStr(CurrIndex + 1), offset) - offset;
      using T = std::tuple_element_t<CurrChar, Types>;
      std::get<CurrChar>(values) = read<T>(view.substr(offset, len));
